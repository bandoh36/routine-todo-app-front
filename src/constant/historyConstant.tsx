...
export const HISTORY_CARD_CONTENT_SIER: HistoryCardProps = {
  ...
  description: (
    <>
      <Typography typography="Noto1">保険系SIerで上流～下流行程を経験</Typography>
      <Typography typography="Noto3" sx={{ p: 3 }}>
        新卒で入社した保険系SIerではウォーターフォールでの開発案件に参画し、
        Javaでのプログラム実装だけでなく、要件定義～運用保守フェーズまで幅広く担当。
        <br />
        <br />
        入社3年目には保守開発プロジェクトのリーダーとして、 案件管理（スケジュール作成／進捗管理）やメンバーのマネジメントも経験。
        また独学・社外の研修を通して、Javascriptを用いたフロントエンド開発や AWSを用いたインフラ環境構築について日々自己学習に取り組む。
      </Typography>
    </>
  ),
};
  ...
  description: (
    <>
      ...
      <Typography typography="Noto3" sx={{ p: 3 }}>
        ...
        <br /> <br />
        また業務外では、当ポートフォリオや学習内容発信のためのブログ、 その他ToDoアプリなど積極的にアウトプットを実施し研鑽中。
      </Typography>
    </>
  ),
};
